...
// leyes.html. Reglas de acceso:
//   * administrador, creador_ong y voluntario ven la burbuja (los 3 roles).
//   * a diferencia del foro (donde el admin solo modera), este es un asistente de consulta
//     (leyes/conocimiento/foro), no un espacio de publicacion entre usuarios, asi que no hay
//     razon para que el admin no pueda usarlo tambien.
// Habla con el endpoint del servidor (POST /api/chat -> Gemini), no toca Supabase directamente
// para leyes (usa window.ONG_LAWS, ya cargado por laws-data.js) y usa window.KMS.api(...) para
// conocimiento y foro, igual que el resto del proyecto.

(function () {
  'use strict';

  const ALLOWED_ROLES = ['creador_ong', 'voluntario', 'administrador'];
  ...
  function init() {
    ...
    if (!widgetAllowed()) return; // solo se muestra a roles permitidos (ver ALLOWED_ROLES)
