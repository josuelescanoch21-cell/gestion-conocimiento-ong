...
async function renderLaws() {
  ...
  renderTree('lawsTree', lawsCategory ? [lawsCategory] : categories.filter((category) => category.name === 'Leyes para ONG'));
  populateCategoryFilters(lawsCategory ? [lawsCategory] : []);
  const items = (await loadKnowledge()).filter((item) => canRead(item) && (item.category === 'Leyes para ONG' || item.type === 'ley' || item.document_type === 'ley'));
  const sub = byId('lawSubcategoryFilter')?.value || '';
  const role = byId('lawRoleFilter')?.value || '';
  const query = (byId('lawSearch')?.value || '').toLowerCase();
  let filtered = items;
  if (sub) filtered = filtered.filter((item) => item.subcategory === sub);
  if (role) filtered = filtered.filter((item) => (item.roles || []).includes(role));
  if (query) filtered = filtered.filter((item) => [item.title, item.description, item.content, item.legal_reference, ...(item.tags || [])].join(' ').toLowerCase().includes(query));
  const target = byId('lawItems');
  if (target) target.innerHTML = filtered.length ? filtered.map(itemCard).join('') : '<div class="empty-state"><strong>No hay normas para este filtro.</strong><p>Admin o Creador ONG pueden crear una nueva ley/norma desde esta seccion.</p></div>';
  const subSelect = byId('lawSubcategoryFilter');
  if (subSelect && lawsCategory && !subSelect.dataset.loaded) {
    subSelect.innerHTML = '<option value="">Todas las subcategorias</option>' + (lawsCategory.subcategories || []).map((subcat) => `<option value="${escapeHtml(subcat.name)}">${escapeHtml(subcat.name)}</option>`).join('');
