...
(function () {
  'use strict';// Burbuja global del asistente IA: disponible como icono flotante en cualquier pantalla del
// sistema (ya no depende de estar en la pestaña "Leyes"). Reemplaza al panel fijo anterior de
// leyes.html. Reglas de acceso:
//   * administrador, creador_ong y voluntario ven la burbuja (los 3 roles).
//   * cambio para el chat IA: antes el administrador quedaba afuera con el mismo criterio del
//     Foro ("modera, no participa"), pero a diferencia del foro este es un asistente de
//     consulta (leyes/conocimiento/foro), no un espacio de publicacion entre usuarios, asi
//     que no hay razon para que el admin no pueda usarlo tambien.
// Habla con el mismo endpoint del servidor (POST /api/ai/legal-chat -> Gemini), no toca
// Supabase directamente para leyes (usa window.ONG_LAWS, ya cargado por laws-data.js) y usa
// window.KMS.api(...) para conocimiento y foro, igual que el resto del proyecto.
